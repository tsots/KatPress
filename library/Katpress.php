...
// Pagination.
if ( ! function_exists( 'katpress_pagination' ) ) :
	function katpress_pagination() {
		global $wp_query;
...
if ( ! function_exists( 'katpress_menu_fallback' ) ) :
	function katpress_menu_fallback() {
		...
		printf(
			...
			sprintf(
				__( '<a href="%s">Menus</a>', 'katpress' ),
				get_admin_url( get_current_blog_id(), 'nav-menus.php' )
			),
			/* translators: %s: customize url */
			sprintf(
				__( '<a href="%s">Customize</a>', 'katpress' ),
...
if ( ! function_exists( 'katpress_active_nav_class' ) ) :
	function katpress_active_nav_class( $classes, $item ) {
		if ( $item->current == 1 || $item->current_item_ancestor == true ) {
			$classes[] = 'is-active';
		}
		return $classes;
	}
	add_filter( 'nav_menu_css_class', 'katpress_active_nav_class', 10, 2 );
endif;
 ...
 */
if ( ! function_exists( 'katpress_active_list_pages_class' ) ) :
	function katpress_active_list_pages_class( $input ) {
	...
	add_filter( 'wp_list_pages', 'katpress_active_list_pages_class', 10, 2 );
endif;
...
if ( ! function_exists( 'katpress_responsive_video_oembed_html' ) ) :
	function katpress_responsive_video_oembed_html( $html, $url, $attr, $post_id ) {
	...
	add_filter( 'embed_oembed_html', 'katpress_responsive_video_oembed_html', 10, 4 );
endif;
...
if ( ! function_exists( 'katpress_mobile_menu_id' ) ) :
	function katpress_mobile_menu_id() {
...
if ( ! function_exists( 'katpress_title_bar_responsive_toggle' ) ) :
	function katpress_title_bar_responsive_toggle() {
